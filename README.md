...
**NOTE:** Pushing plugins using this command is only available when the wordpress site that this plugin is installed on is publically available. This is because the server needs to reach out to the plugin site to get the plugin data that was zipped up. 
